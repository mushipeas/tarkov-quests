...
class QuestSpider(scrapy.Spider):
    name = "questspider"
    start_urls = [
        "https://escapefromtarkov.gamepedia.com/index.php?title=Special:WhatLinksHere/Quests&limit=500"
    ]

    def parse(self, response):
        yield from response.follow_all(
            css="#mw-whatlinkshere-list > li > a", callback=self.parse_subpage
        )
    ...
    def parse_subpage(self, response):
        ...
        if pagetype == "Quests":
            ...
            if response.css("#Requirements::text").get():
                requirements = (
                    response.xpath(
                        'string(//div[contains(@id,"mw-content-text")]/div/ul)'
                    )
                    .get()
                    .split("\n")
                )
            ...
            rewards = (
                response.xpath(
                    'string(//div[contains(@id,"mw-content-text")]/div/ul[{n}])'.format(
                        n=2 + ul_shift
                    )
                )
                .get()
            ...
            metadata = response.css(
                "#va-infobox0-content > td > table:nth-child(3) > tbody > tr > td.va-infobox-label::text"
            ).getall()
            metadata_vals = [
                a.xpath("string()").get().split(", ")
                for a in response.css(
                    "#va-infobox0-content > td > table:nth-child(3) > tbody > tr > td.va-infobox-content"
                )
            ]
            ...
            yield {
                ...
                "previous": response.css(
                    "#va-infobox0-content > td > table:nth-child(5) > tbody > tr:nth-child(4) > td:nth-child(1) > a::text"
                ).getall(),
                "leads_to": response.css(
                    "#va-infobox0-content > td > table:nth-child(5) > tbody > tr:nth-child(4) > td:nth-child(3) > a::text"
                ).getall(),
